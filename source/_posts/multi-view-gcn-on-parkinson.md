...
图1展示作者开发的基于多视图脑图的关系预测的MVGCN整体框架。作者的模型是一个由三个主要部分组成的深度神经网络：第一个部分是一个多视图的GCN，通过每次采集来提取特征矩阵；第二个部分是一个两两匹配的策略，用于将每次采集到了的特征矩阵对聚合到一个特征向量；第三部分是一个用于关系预测的softmax预测。所有这些部分都通过反向传播和随机优化进行训练。需要注意的是，MVGCN是一个端到端的结构，在视图池和两两匹配中没有引入额外的参数。并且在多视图GCN部分，视图中的所有分支都共享相同的参数。接下来，作者将对每个组件进行详细解释。
...
对于每一对，成对匹配层的输出是特征向量$\mathbf{r}$，其中每个元素由式子4给出。然后，将表达传递给全连接层的Softmax层进行分类。它计算了标签的概率分布(式子5)：
...
为了学习图之间的相似性，在同一组的脑网络（PD或HC）标记为匹配对，而将不同组的脑网络标记为非匹配对。因此，总共就有283881对，其中189713是匹配样本，94168是非匹配样本。所有实验将样本分为5个随机数据集，并采用5次交叉验证。利用DTI中ROI的坐标信息构建一个具有84个顶点和527条边的10-NN BGG。对于图卷积层，使用$s=30$阶的切比雪夫多项式并输出的特征维度为$F_{out}=128$。对于全链接层，特征尺度在全链接层的维度是1024，然后再是一个64维度的全链接层。使用Adam优化器，初始学习率为0.005。通过交叉验证，以上参数是所有方法的最佳设置。MVGCN代码和脚本在公共存储库中可用。[代码](https://github.com/sheryl-ai/MVGCN)

## 实验结果

因为作者的目标是预测成对BCGs之间的关系（匹配与非匹配的对象），因此使用ROC的下面积（AUC）来进行评估。表1提供使用以下方法的各个结果视图：原始边权重、PCA、前馈全链接网络（$FCN$和$FCN_{2l}$）和图卷积网络（GCN），其中$FCN_{2l}$表示两层FCN。通过比较方法，可以学习到每一个成对对象的特征表示。为了便于比较，所有方法都使用了成对匹配组件和软件组件。基于GCN的方法达到了最佳的性能93.90%的AUC。结果表明，GCN方法优于原始边权重的方法、传统的线性降维PCA、非线性神经网络$FCN$和$FCN_{2l}$。

表1：根据AUC度量对匹配和非匹配脑网络进行分类的结果

{% asset_img result-for-classifying.png 根据AUC度量对匹配和非匹配脑网络进行分类的结果 %}

表2反映了作者提出的MVGCN的三个基线在分类和获取聚类方面的性能。提出了基于隐藏层输出维数的神经网络结构。$M$表示式子4中的匹配层，$S$表示式子5中的Softmax操作。数字表示在不同层提取的特征的维数。在作者的研究中，作者在视图池组件中评估了元素最大池和平均池。具体来说，为了测试学习到的相似性的有效性，作者还根据NMI评估了聚类的性能。作者使用的采集聚类算法是K-means（$K=2$，PD和HC）。结果表明，作者的MVGCN在分类和获取聚类任务上均优于所有基线，AUC为95.37%，NMI为1.00。为了验证预测结果对区分PD和HC脑网络是否有意义，作者对给定的754个DTI数据进行了欧氏距离可视化。由于所有匹配模型的输出值都能反映两次采集之间的成对相似性，因此作者用t-SNE将其映射到二维空间。图3比较了不同方法的可视化结果。PCA特征提取不能很好地分离PD和HC。在具有最佳AUC的ODF-RK2视图中，$FCN_{2l}$的结果要好得多，并且可以在少量重叠采集的情况下观察到两个簇。与PCA和$FCN_{2l}$相比，最大视图池MVGCN的可视化结果清晰地显示了两个分离良好且相对紧凑的组。

表2：使用单视图和多视图结构比较二分类（AUC）和采集聚类（NMI）

{% asset_img single-view-and-multi-view.png 使用单视图和多视图结构比较二分类（AUC）和采集聚类（NMI） %}

{% asset_img visualization-dti.png DTI采集集群的可视化 %}

图3：DTI采集集群的可视化。以两两关系的预测值为输入，采用t-SNE算法将采集数据映射到二维空间。蓝色表示PD，红色表示HC。

此外，我们还研究了所提MVGCN的成对特征向量。在基于ROI-ROI的成对匹配之后，每对的输出是一个特征向量，嵌入给定两次采集的相似性，每个元素都与一个ROI相关联。通过可视化ROI上的值分布，可以解释为模型学习到的成对特征向量。图4显示PD或HC组最相似或不同的10个ROI。直接从成对匹配层的输出表示中提取相似度。作者计算某些群的平均值。例如，在给定成对PD样本的情况下计算相似度分布，图4（a）中示出了前10个ROI的值。结果表明，侧眶额区、颞中区和杏仁核区是PD患者最相似的3个ROI，而尾状核和壳核区等重要ROI对PD和HC有鉴别作用（见图4（d））。观察结果表明，学习到的成对特征向量与一些临床发现一致，从而验证了MVGCN用于神经图像分析的有效性。

{% asset_img visualization-roi-roi.png 可视化学习到的ROI-ROI相似性 %}

图4：从特定组中的平均成对特征向量中可视化学习到的ROI-ROI相似性。PD组和HC组的前10个相似或不同ROI以及相应的值分别显示在（a）-（d）中。

# 讨论

该方法的基本原理是同时建立基于共同感兴趣区域坐标的多脑连接性网络（BCGs）和几何图形（BGG）模型。由于BCGs是非欧几里德的，因此使用标准的卷积网络是不容易的。此外，多视图图融合方法允许探索给定数据的各个方面。作者的非参数化视图池在实践中很有希望。此外，成对学习策略只需很少的采集就可以满足“数据饥渴“的神经网络。作者的工作证明了图神经网络在多图结构神经图像场景中的强大潜力。同时，作者的方法所学习到的表征可以被直接解释。但是，仍然存在一些局限性。目前的方法完全是数据驱动的，不需要利用任何临床领域的知识。在疾病分析中不考虑电子健康记录等临床数据。今后，我们将继续沿着这些方向进行研究。

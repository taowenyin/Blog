...
类数量比较大就意味着许多类只有较少的实例。因此就需要学习一组精确的规则，不是一个完整的分类器。这种数据的特殊性使得作者有必要开发一种基于bootstrap的复杂重采样方法来估计规则的精度。所有的精度测量都是使用m-estimate计进行的，m-estimate是拉普拉斯估计的推广，考虑到了类别的先验概率。m-estimate的规则如下：

$$M(r)=\frac{p+m \frac{P}{P+N}}{p+n+m}$$

其中$P$表示所有实例中的正例，$N$表示所有实例中的反例，$p$表示规则$r$所涵盖的正例数量，$n$表示规则$r$所涵盖的负例数量，$m$表示要更改的参数。

使用这个公式，零覆盖规则的精度将是类的先验概率。$m$是一个参数，可以改变它来加权先验概率。作者使用$m=1$。
...
作者的目的是学习可以从生物学角度解释的预测函数类别的规则。为此，作者将数据集分为三部分：训练数据、从中选择最佳规则的验证数据（选择的规则精度至少为50%，正确覆盖至少两个示例）和测试数据。作者使用验证数据来避免对数据过度拟合。然而，将数据集分成3部分意味着可用于训练的数据量将更少。同样地，只有少量可用于测试。最初的实验表明，数据的分割对产生的规则集有很大的影响，有时产生许多好的规则，有时则没有。在小数据集的情况下精度估计有两种方法，分别是是交叉验证和bootstrap方法。因为作者对规则本身感兴趣，而不仅仅是对准确性感兴趣，因此作者选择了bootstrap方法，因为交叉验证只是生成10个规则集，而bootstrap采样可用于创建数百个数据样本，从而生成数百个规则集。然后，可以检查这些规则，并查看哪些规则定期出现并且是稳定的，而不仅仅是数据分割的产物。

bootstrap是一种重复采样并替换数据从而产生百个训练集的方法。为每个样本构造一个分类器，并对所有分类器的精度进行平均以给出最终的精度度量。第一次通过在原始数据中通过bootstrap采样，样本中未使用的原始数据项构成了测试集，然后从样本中取出新的样本并替换。第二次采样用于训练数据，第一个采样中的样本（而不是第二个样本中的项目）构成了验证集。这样三个数据集都不重叠。
